...
//
// Include this AFTER the implementation (#include "karma~.c" / "k4~.c" /
// "karma_core.h") so that `t_karma` resolves to that implementation. Everything
// here touches only the shared public API (karma_record/play/stop/overdub/
// append/jump/float/select_*) plus a perform dispatch supplied by the driver,
// never implementation-specific struct fields -- so the same code drives the
// reference, k4, and the extracted core.
...
// Driver-supplied perform dispatch: routes to mono/stereo/quad (or poly) by the
// object's own channel count. Wraps the 9-arg perform signature.
typedef void (*perform_fn)(t_karma *x, double **ins, long nins,
                           double **outs, long nouts, long vcount);

// Driver-supplied constructor: fresh object bound to a zeroed buffer.
...
static void run_scenario(t_karma *x, const scenario *sc, perform_fn perform, FILE *out)
{
    ...
    ins[chans] = in_speed;             // speed is the last signal inlet
    long nins = chans + 1, nouts = chans;

    long    n_out = sc->total * chans; // interleaved output capture
