...
struct t_karma {
    ...
    double  *poly_writeval;     // ipoke per-channel write accumulators (persist across vectors)
...
// New helper functions for reducing code duplication in perform routines
static inline float* kh_setup_perform_buffer(
    t_karma* x, t_buffer_obj** buf_out, t_bool* record, t_bool* recordprev,
    t_bool* dirt);

static inline void kh_cleanup_perform(
    t_karma* x, t_buffer_obj* buf, float* b, t_bool dirt, t_bool go);

static inline void kh_process_initial_loop_playfade(
    double globalramp, long* playfade, char* playfadeflag,
    char* recendmark, t_bool* go);
...
static inline double kh_spline_interp(double f, double w, double x, double y, double z)
{
    return (((-0.5*w + 1.5*x - 1.5*y + 0.5*z)*f*f*f) + ((w - 2.5*x + y + y - 0.5*z)*f*f) + ((-0.5*w + 0.5*y)*f) + x);
}

// Interpolation kernels as macros, identical to the reference karma~.c. Used in the
// perform routines instead of the kh_*_interp functions so the compiler contracts FMA
// the same way the reference does -- this keeps the perform output bit-exact against the
// reference (the inline-function form contracts differently, giving ~1e-9 drift).
        ...
        if (x->state.recendmark) {  // calculate end of loop
            if (x->state.directionorig >= 0) {
                ...
                *setloopsize = x->loop.maxloop - x->loop.minloop;
                ...
                x->loop.endloop = x->loop.startloop + (x->timing.selection * (*setloopsize));   // recompute endloop from new startloop/selection (matches forward branch + original karma~.c)
...
void* karma_new(t_symbol* s, short argc, t_atom* argv)
{
    ...
    */  }
    ...
    if (x) {
        ...
        x->poly_writeval = (double*)sysmem_newptrclear(x->poly_maxchans * sizeof(double));
        if (!x->poly_writeval) {
            object_error((t_object*)x, "Failed to allocate memory for multichannel processing arrays");
            sysmem_freeptr(x->poly_osamp);
            sysmem_freeptr(x->poly_oprev);
        ...
        */
        ...
        if (!x->messout) {
            ...
            sysmem_freeptr(x->poly_odif);
            sysmem_freeptr(x->poly_recin);
            sysmem_freeptr(x->poly_writeval);
            object_free((t_object*)x);
            return NULL;
        }
...
void karma_free(t_karma* x)
{
    if (x->state.initskip) {
        ...
        if (x->poly_writeval) sysmem_freeptr(x->poly_writeval);
    ...
    long vcount, long flgs, void* usr)
{
    ...
    // Self-contained mono perform (ported verbatim from the reference karma~.c mono
    // routine): all per-vector state lives in locals loaded here and stored at the end,
    // so there is no x-> / local snapshot desync. Uses the kh_ low-level primitives.
    t_buffer_obj* buf;
    ...
    while (n--)
    {
        ...
        if ((record - recordprev) < 0) {           // samp @record-off
            ...
            if (speed < 1.0)
                snrfade = 0.0;
            if (globalramp)
                kh_ease_bufoff(frames - 1, b, pchans, accuratehead, -direction, globalramp);
        }
        recordprev = record;
        ...
        if (!loopdetermine)
        {
            if (go)
            {
                ...
                */
                        ...
                        if (wrapflag)
                        {
                            if ((accuratehead > endloop) && (accuratehead < startloop))
                            {
                                ...
                                if (accuratehead > maxloop)
                                {
                                    ...
                                    if (record) {
                                        if (globalramp) {
                                            kh_ease_bufoff(frames - 1, b, pchans, maxloop, -direction, globalramp);
                                            recordfade = 0;
                                        }
                                        recfadeflag = 0;
                                        recordhead = -1;
                                ...
                                else if (accuratehead < 0.0)
                                {
                                    ...
                                    if (record) {
                                        if (globalramp) {
                                            kh_ease_bufoff(frames - 1, b, pchans, minloop, -direction, globalramp);     // 0.0  // ??
                                            recordfade = 0;
                                        }
                                        recfadeflag = 0;
                                        recordhead = -1;
                                ...
                                if (accuratehead < ((frames - 1) - maxloop))
                                {
                                    ...
                                    {
                                        if (globalramp) {
                                            kh_ease_bufoff(frames - 1, b, pchans, ((frames - 1) - maxloop), -direction, globalramp);
                                            recordfade = 0;
                                        }
                                        recfadeflag = 0;
                                        recordhead = -1;
                                    ...
                                    if (record) {
                                        if (globalramp) {
                                            kh_ease_bufoff(frames - 1, b, pchans, (frames - 1), -direction, globalramp);
                                            recordfade = 0;
                                        }
                                        recfadeflag = 0;
                                        recordhead = -1;
                ...
                kh_interp_index(playhead, &interp0, &interp1, &interp2, &interp3, direction, directionorig, maxloop, frames - 1);  // samp-indices
                ...
                {                                           // "Switch and Ramp" - http://msp.ucsd.edu/techniques/v0.11/book-html/node63.html
                    if (snrfade < 1.0)
                    {
                        ...
                        osamp1 += kh_ease_switchramp(o1dif, snrfade, snrtype);// <- easing-curv options implemented by raja
                        snrfade += 1 / snrramp;
                    }                                               // "Switch and Ramp" end
...
//                          break;                                  // !! no break - pass 2 -> 3 !!
            ...
            *out1++ = osamp1;
            ...
            /*
             ...
             recin1 = kh_ease_record(recin1 + (b[playhead * pchans] * overdubamp), recfadeflag, globalramp, recordfade); ...
            ...
            */
            if (record)
            {
                if ((recordfade < globalramp) && (globalramp > 0.0))
                    recin1 = kh_ease_record(recin1 + (((double)b[playhead * pchans]) * overdubamp), recfadeflag, globalramp, recordfade);
                else
...
apned:
                    ...
                    if (direction == directionorig)                 // buffer~ boundary constraints and registry of maximum distance traversed
                    {
                        if (accuratehead > (frames - 1))
                        {
                            ...
                            if (record) {
                                if (globalramp) {
                                    kh_ease_bufoff(frames - 1, b, pchans, (frames - 1), -direction, globalramp);   // maxloop ??
                                    recordhead = -1;
    ...
    long vcount, long flgs, void* usr)
{
    ...
    // Self-contained stereo perform (ported verbatim from the reference karma~.c stereo
    // routine): all per-vector state lives in locals, so there is no x-> / local snapshot
    // desync. Uses the kh_ low-level primitives and the INTERP macros.
    t_buffer_obj* buf;
    float* b = kh_setup_perform_buffer(x, &buf, &record, &recordprev, &dirt);
    if (!b)
        goto zero;
        ...
        while (n--)
        {
            ...
            if (!loopdetermine)
            {
                if (go)
                {
                    ...
                    kh_interp_index(playhead, &interp0, &interp1, &interp2, &interp3, direction, directionorig, maxloop, frames - 1);  // samp-indices for interp
...
//                              break;                                  // !! no break - pass 2 -> 3 !!
                ...
                *out2++ = osamp2;
                ...
                if (record)
                {
                    if ((recordfade < globalramp) && (globalramp > 0.0)) {
                        recin1 = kh_ease_record(recin1 + (((double)b[playhead * pchans]) * overdubamp), recfadeflag, globalramp, recordfade);
                        recin2 = kh_ease_record(recin2 + (((double)b[(playhead * pchans) + 1]) * overdubamp), recfadeflag, globalramp, recordfade);
                    } else {
                        recin1 += ((double)b[playhead * pchans]) * overdubamp;
                    ...
                    apned:
                        ...
                        if (direction == directionorig)                    // buffer~ boundary constraints and registry of maximum distance traversed
                        {
                            if (accuratehead > (frames - 1))
                            {
                                ...
                                if (record) {
                                    if (globalramp) {
                                        kh_ease_bufoff(frames - 1, b, pchans, 0.0, -direction, globalramp);
                                        recordhead = -1;
                                        recfadeflag = recordfade = 0;
                                    }
                                }
...
{
    long    syncoutlet = x->syncoutlet;
    long    ochans     = x->buffer.ochans;

    if (ochans > x->poly_maxchans)
        ochans = x->poly_maxchans;
#if KARMA_VALIDATE_CHANNEL_BOUNDS
    if (ochans > KARMA_ABSOLUTE_CHANNEL_LIMIT) {
        error("karma~: Channel count %ld exceeds maximum configured channels (%d)",
              ochans, KARMA_ABSOLUTE_CHANNEL_LIMIT);
        ochans = KARMA_ABSOLUTE_CHANNEL_LIMIT;
    }
#endif
    ...
    // Self-contained channel-generic poly perform (ported verbatim from karma_core's
    // unified perform; the reference shipped unrolled mono/stereo/quad). All per-vector
    // state is in locals; the per-channel oprev/odif/writeval accumulators live in the
    // persistent x->poly_* arrays. No x-> / local snapshot desync.
    //
    // MC routing: audio ins[0..ochans-1], speed ins[ochans]; with a sync outlet,
    // outs[0]=sync and audio starts at outs[1], else audio at outs[0..ochans-1].
    double *in[KARMA_ABSOLUTE_CHANNEL_LIMIT], *out[KARMA_ABSOLUTE_CHANNEL_LIMIT];
    long    out_base = syncoutlet ? 1 : 0;
    for (ch = 0; ch < ochans; ch++) { in[ch] = ins[ch]; out[ch] = outs[out_base + ch]; }
    double *inspeed = ins[ochans];
    double *outPh   = syncoutlet ? outs[0] : 0;

    double accuratehead, maxhead, jumphead, srscale, speedsrscaled, recplaydif, pokesteps;
    double speed, speedfloat, overdubamp, overdubprev, ovdbdif, selstart, selection;
    double frac, snrfade, globalramp, snrramp;
    double coeff[KARMA_ABSOLUTE_CHANNEL_LIMIT];
    double *osamp = x->poly_osamp, *recin = x->poly_recin, *writeval = x->poly_writeval;
    double *oprev = x->poly_oprev, *odif = x->poly_odif;
    t_bool go, record, recordprev, alternateflag, loopdetermine, jumpflag, append, dirt, wrapflag, triginit;
    char direction, directionprev, directionorig, recendmark, playfadeflag, recfadeflag;
    control_state_t   statecontrol;
    switchramp_type_t snrtype;
    interp_type_t     interp;
    long playfade, recordfade, i, interp0, interp1, interp2, interp3, pchans, nproc;
    long frames, startloop, endloop, playhead, recordhead, minloop, maxloop, setloopsize;
    long initiallow, initialhigh;
            ...
            if (go)
            {
                ...
                {                                           // "Switch and Ramp" - http://msp.ucsd.edu/techniques/v0.11/book-html/node63.html
                    if (snrfade < 1.0)
                    {
                        ...
                        for (ch = 0; ch < nproc; ch++)
                            osamp[ch] += kh_ease_switchramp(odif[ch], snrfade, snrtype);// <- easing-curv options implemented by raja
                        snrfade += 1 / snrramp;
                    }                                               // "Switch and Ramp" end
                    ...
                    {                                               // realtime ramps for play on/off
                        for (ch = 0; ch < nproc; ch++)
                            osamp[ch] = kh_ease_record(osamp[ch], (playfadeflag > 0), globalramp, playfade);
                        playfade++;
                        if (playfade >= globalramp)
                        {
                            switch (playfadeflag)
...
//                          break;                                  // !! no break - pass jump -> recoff !!
            ...
            */
            if (record)
            {
                for (ch = 0; ch < nproc; ch++) {
                    if ((recordfade < globalramp) && (globalramp > 0.0))
                        recin[ch] = kh_ease_record(recin[ch] + (((double)b[playhead * pchans + ch]) * overdubamp), recfadeflag, globalramp, recordfade);
                    else
...
/**
 * @brief Sets up buffer for perform routine - reduces code duplication
 *
 * This helper function extracts the common pattern of buffer setup and validation
 * that appears at the start of all three perform routines.
 *
 * @param x The karma~ object instance
 * @param buf_out Output parameter for buffer object
 * @param record Output parameter for record state
 * @param recordprev Output parameter for previous record state
 * @param dirt Output parameter for dirt flag
 * @return Pointer to locked buffer samples, or NULL if invalid
 */
static inline float* kh_setup_perform_buffer(
    t_karma* x, t_buffer_obj** buf_out, t_bool* record, t_bool* recordprev,
    t_bool* dirt)
{
    ...
    *dirt = 0;
    ...
    if (!b || x->k_ob.z_disabled) {
        return NULL; // Caller should goto zero
    }
...
/**
 * @brief Cleans up after perform routine - reduces code duplication
 *
 * This helper function extracts the common pattern of buffer cleanup and
 * clock handling that appears at the end of all three perform routines.
 *
 * @param x The karma~ object instance
 * @param buf Buffer object to unlock
 * @param b Buffer samples pointer (unused, for symmetry)
 * @param dirt Whether buffer was modified
 * @param go Current go state for clock handling
 */
static inline void kh_cleanup_perform(
    t_karma* x, t_buffer_obj* buf, float* b, t_bool dirt, t_bool go)
{
...
/**
 * @brief Processes playfade state during initial loop creation
 *
 * This helper function extracts the duplicated playfade handling logic
 * that appears in the initial loop creation section of perform routines.
 * Handles both globalramp and no-globalramp cases.
 *
 * @param globalramp Global ramp samples setting
 * @param playfade Pointer to playfade counter
 * @param playfadeflag Pointer to playfade flag
 * @param recendmark Pointer to recording end marker
 * @param go Pointer to go state
 */
static inline void kh_process_initial_loop_playfade(
    double globalramp, long* playfade, char* playfadeflag,
    char* recendmark, t_bool* go)
